...
sns.countplot(data=df, x='tenure time', hue='Churn')
plt.show()
...
# Calculate average MonthlyCharges and TotalCharges for churn and non-churn groups
...
# Monthly Charges
plt.subplot(1, 2, 1)
avg_monthly_charges.plot(kind='line', marker='o', color=['skyblue', 'lightcoral'])
...
# Total Charges
plt.subplot(1, 2, 2)
avg_total_charges.plot(kind='line', marker='o', color=['skyblue', 'lightcoral'])
...
# Selecting only the categorical columns
categorical_columns1 = ['gender', 'SeniorCitizen', 'Partner' ]
...
categorical_columns12 = ['Dependents', 'TechSupport']
num_cols = len(categorical_columns12)
fig, axes = plt.subplots(nrows=num_cols, ncols=1, figsize=(15, 5 * num_cols))

# Loop through each column and create a count plot
for i, column in enumerate(categorical_columns12):
    sns.countplot(x=column, hue="Churn", data=df, ax=axes[i])
    axes[i].set_title(f'Countplot for {column}')
    axes[i].set_xlabel('')  # Remove x-axis label for better organization
...
# Calculate the correlation matrix with respect to 'Churn'
churn_correlation = df2.corr()['Churn'].drop('Churn')

# Plot the correlation with 'Churn'
plt.figure(figsize=(8, 6))
sns.heatmap(churn_correlation.to_frame(), annot=True, cmap='coolwarm', fmt=".2f", cbar=False)
plt.title('Correlation with Churn')
plt.savefig('churn_correlation_heatmap.png')  # Save the plot as a PNG file
plt.show()
...
dump(model1, 'logistic_regression_model.joblib')

from joblib import dump

# Save the model to a file in a specific directory
dump(model1, 'C:\\Users\\HP\\OneDrive\\Desktop\\ml project\\logistic_regression_model.joblib')
